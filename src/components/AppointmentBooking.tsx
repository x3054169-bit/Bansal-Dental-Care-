...
const treatments = [
  ...
  "Painless Treatments",
  "Preventive Care",
  "Dental Procedures (modern techniques)",
...
const AppointmentBooking = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  const handleBooking = async () => {
    ...
    const message = `Hi, I'd like to book an appointment with Dr Ashutosh Bansal for ${formData.treatment} at ${formData.time} on ${formData.date}. Name: ${formData.name}, Phone: ${formData.phone}`;
    const whatsappUrl = `https://wa.me/919782689909?text=${encodeURIComponent(message)}`;
